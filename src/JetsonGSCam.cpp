...
namespace jetson_gscam {
    ...
    void JetsonGSCam::onInit() {
        ...
        camera_info_url = nh.param<std::string>("camera_info_url", "");
        camera_name = nh.param<std::string>("camera_name", "camera");
        ...
        // setup camera info
        camera_info_manager = new camera_info_manager::CameraInfoManager(nh, camera_name, camera_info_url);
        if (!camera_info_manager->isCalibrated()) {
            NODELET_WARN("Camera not calibrated ");
        }
...
//            gst_object_unref(bus);
    ...
    bool JetsonGSCam::setupPipeline() {
        ...
        gst_cmd << "nvarguscamerasrc sensor_id=" << sensor_id << " ! "
                ...
                << "video/x-raw(memory:NVMM),width=" << width << ",height=" << height << ",framerate=" << fps
                << "/1,format=NV12 ! "
                << "nvjpegenc ! "
...
//                gst_element_get
    ...
    void JetsonGSCam::publisher() {
        ...
        while (pub_thread_running && !ros::isShuttingDown()) {
            ...
            if (got_sample) {
                if (camera_info_manager) {
                    auto cam_info = camera_info_manager->getCameraInfo();
